...
var config = {  //ok
  ...
  readInterval: 1000,
  maxSendErrors: 5,
  ignoreReadBeforeInit: true,
  ignoreWriteBeforeInit: true
};
...
var init = function(c, callback) {
    ...
    var allowedAttrs = ['zipFile', 'wagoAddress', 'maxSendErrors', 'readInterval', 'ignoreReadBeforeInit', 'ignoreWriteBeforeInit'];
    for (var attr in c) {
...
var executeReadQueue = function() {
  if (Debug >= 2) console.log("executeReadQueue -> checking Read Queue. Items to serve: " + readQueue.length);
...
var addToReadQueue = function(addr, callback) {
  ...
  if (Debug == 2) console.log("addToReadQueue -> Received request for " + addr);
  
  if (!initComplete) {  //what to do when init not completed?
    if (config.ignoreReadBeforeInit) {
      ...
      status.on('initComplete', function() {addToReadQueue(addr, callback)});  //<- callback will be executed later...
    }
  } 
  else {
    if (typeof addr !== 'undefined' && addr.length > 0) {
      ...
      if (Debug == 2) console.log("addToReadQueue -> Error, wrong or no address given");
      callback('Error', 'adr!');
    }
  }
...
var readData = function(addrs, callback) {
  ...
  if (Debug == 2) console.log("readData -> received request for " + addrs);
  ...
  if (!initComplete) {  //what to do when init not completed?
    if (config.ignoreReadBeforeInit) {
      ...
      status.on('initComplete', function() {readData(addrs, callback)});  //<- callback will be executed later...
    }
  }
  else {
    ...
    if (typeof addrs !== 'undefined' && addrs.length > 0) {
      ...
      if (Debug == 2) console.log("readData -> Error, wrong or no address given");
      callback('Error', 'adr!');
    }
...
var writeData = function(addr, val, callback) {
  if (Debug == 1) console.log('writeData -> received request for: ' + addr + ', value: ' + val);

  if (!initComplete) {  //what to do when init not completed?
    if (config.ignoreWriteBeforeInit) {
      if (Debug == 2) console.log('writeData -> Init not completed, ignoring request');
      callback('Error', 'notReady');
    }
    else {
      if (Debug == 2) console.log('writeData -> Init not completed, adding request to queue');
      status.on('initComplete', function() {writeData(addr, val, callback)});  //<- callback will be executed later...
    }
  }  
  else {
    if (typeof(addr) !== 'undefined' && addr.length > 0) {
        ...
        request({
          ...
          function(error, response, body) {
            ...
            if (!error && response.statusCode == 200 && body =='|0|') { //all was fine!
              if (Debug == 1) console.log("writeData -> success! Received proper reply");
              sendErrors = 0;  //reset error count
              
              //tell the caller we are done...
              callback(null, 'ok');  // 'ok' is important for complex functions like tap()
              ...
              if (sendErrors < config.maxSendErrors) {
                ...
                if (Debug == 1) console.log("writeData -> Maximum error number exceeded, stopping");
                callback('Error', 'maxErr'); 
              }
            } // Eof result handling
